# 📬 Gmail Digest
> 마지막 업데이트: 2026-07-01 06:27 KST

## 📊 오늘 현황

| 카테고리 | 건수 |
|---------|------|
| ✅ 승인 | 2 |
| ❌ 반려 | 0 |
| 🚨 에러 | 92 |
| ⚠️ 경고 | 0 |
| 📊 알림 | 7 |

## ✅ 승인

- `11:16` [앱인토스] 계절 꽃 도감 v20260626-5 → ✅ 승인 ⚠️ 출시 업데이트 필요
- `10:51` [앱인토스] 1분 경제뉴스 v20260627-17 → ✅ 승인 ⚠️ 출시 업데이트 필요

## 🚨 에러

### 🔴 GitHub Actions 실패

| 레포 | 워크플로우 | 타입 | 횟수 | 커밋 |
|------|-----------|------|------|------|
| `` |  | 기타 | **14회** | `` |
| `2424` | Live Cost Watch | 기타 | **2회** | `5779ca2` |
| `dog-breed` | Update BLS Cost Data | 기타 | 1회 | `29305ed` |
| `gungangbohum` | Publish scheduled content | 기타 | **14회** | `d805895` |
| `runmania` | Auto-Publish Cron | 기타 | 1회 | `607ed16` |
| `askorekr` | plant-data-pipeline | 데이터 | **2회** | `1e235ed` |
| `campgogo.kr` | bulk-collect | 데이터 | 1회 | `2aa94af` |
| `nongsusangogo.kr` | bulk-collect | 데이터 | **2회** | `45d9655` |
| `gmail-digest` | Gmail Digest | 배포 | 1회 | `68a5e65` |
| `campgogo.kr` | backup | 시스템 | **2회** | `2aa94af` |
| `gong365kr` | StartupMoneyMap cron | 시스템 | **26회** | `bdd5a4e` |
| `today_yakuk` | Turso Usage Monitor | 시스템 | 1회 | `6f7b14c` |
| `crepikacom` | Auto Publish Blog Post | 콘텐츠 | **16회** | `9430fa4` |
| `crepikacom` | Auto Publish Utility Tool | 콘텐츠 | **9회** | `9430fa4` |

## 📊 알림

- `18:21` [공공데이터]  공공데이터포털 
- `14:33` [공공데이터]  공공데이터포털 
- `13:36` [공공데이터]  공공데이터포털 
- `09:19` [공공데이터]  공공데이터포털 
- `15:55` [공공데이터]  공공데이터포털 
- `15:16` [공공데이터]  공공데이터포털 
- `12:41` [GSC] gradienttrail.com

## 📁 로그 파일

- [2026-07-01](logs/2026-07-01.md)
- [2026-06-30](logs/2026-06-30.md)
- [2026-06-29](logs/2026-06-29.md)
- [2026-06-28](logs/2026-06-28.md)
- [2026-06-27](logs/2026-06-27.md)
- [2026-06-26](logs/2026-06-26.md)
- [2026-06-25](logs/2026-06-25.md)
- [2026-06-24](logs/2026-06-24.md)
- [2026-06-23](logs/2026-06-23.md)
- [2026-06-22](logs/2026-06-22.md)
- [2026-06-21](logs/2026-06-21.md)
- [2026-06-20](logs/2026-06-20.md)
- [2026-06-19](logs/2026-06-19.md)
- [2026-06-18](logs/2026-06-18.md)
