# 📬 Gmail Digest
> 마지막 업데이트: 2026-06-28 05:07 KST

## 📊 오늘 현황

| 카테고리 | 건수 |
|---------|------|
| ✅ 승인 | 0 |
| ❌ 반려 | 0 |
| 🚨 에러 | 169 |
| ⚠️ 경고 | 0 |
| 📊 알림 | 0 |

## 🚨 에러

### 🔴 GitHub Actions 실패

| 레포 | 워크플로우 | 타입 | 횟수 | 커밋 |
|------|-----------|------|------|------|
| `` |  | 기타 | **16회** | `` |
| `2424` | Live Cost Watch | 기타 | **5회** | `e387ee2` |
| `campgogo.kr` | Live Cost Watch | 기타 | **2회** | `87eee46` |
| `crepikacom` | Quality Gates | 기타 | **5회** | `3ad04d7` |
| `crepikacom` | Live Cost Watch | 기타 | **2회** | `14df2ad` |
| `dog-breed` | Update Outdoor Risk Data | 기타 | 1회 | `ceb35ca` |
| `dog-breed` | Publish scheduled BreedWise posts | 기타 | 1회 | `547b4b3` |
| `dolbomjigi` | Live Cost Watch | 기타 | **2회** | `5d2230a` |
| `gong365kr` | Live Cost Watch | 기타 | **2회** | `f5b3ca3` |
| `gungangbohum` | Publish scheduled content | 기타 | **16회** | `d805895` |
| `lim-skills` | Skill checks | 기타 | 1회 | `2fd4b39` |
| `nongsusangogo.kr` | Live Cost Watch | 기타 | **2회** | `15579a3` |
| `petjigi` | Live Cost Watch | 기타 | **2회** | `d1c6a39` |
| `pickleball_app` | Live Cost Watch | 기타 | **2회** | `2375168` |
| `roadwayskr` | Live Cost Watch | 기타 | **2회** | `4202ecd` |
| `spinkorea` | Live Cost Watch | 기타 | **2회** | `89c62c3` |
| `temon` | Live Cost Watch | 기타 | **2회** | `3f3b0bb` |
| `tennis_friends` | Live Cost Watch | 기타 | **2회** | `ff01f4d` |
| `today_yakuk` | Live Cost Watch | 기타 | **2회** | `9f04c33` |
| `yoyangwon` | Live Cost Watch | 기타 | **2회** | `8f8d15d` |
| `yungyanggogo.kr` | Live Cost Watch | 기타 | **2회** | `61830ee` |
| `askorekr` | plant-data-pipeline | 데이터 | **2회** | `1e235ed` |
| `nongsusangogo.kr` | bulk-collect | 데이터 | **2회** | `2f463a5` |
| `2424` | Deploy to Vercel | 배포 | **6회** | `5779ca2` |
| `dolbomjigi` | CI | 배포 | **5회** | `84e11e1` |
| `pickleball_app` | Lint TS (eslint, prettier) | 배포 | **5회** | `259c9cb` |
| `tennis_friends` | Deploy to GitHub Pages | 배포 | **5회** | `2c21087` |
| `campgogo.kr` | backup | 시스템 | **2회** | `2aa94af` |
| `gong365kr` | StartupMoneyMap cron | 시스템 | **31회** | `bdd5a4e` |
| `today_yakuk` | Turso Usage Monitor | 시스템 | **2회** | `4184166` |
| `crepikacom` | Auto Publish Blog Post | 콘텐츠 | **21회** | `cba5070` |
| `crepikacom` | Auto Publish Utility Tool | 콘텐츠 | **13회** | `cba5070` |

- `00:32` [Vercel] 배포 실패
- `00:21` [Vercel] 배포 실패

## 📁 로그 파일

- [2026-06-28](logs/2026-06-28.md)
- [2026-06-27](logs/2026-06-27.md)
- [2026-06-26](logs/2026-06-26.md)
- [2026-06-25](logs/2026-06-25.md)
- [2026-06-24](logs/2026-06-24.md)
- [2026-06-23](logs/2026-06-23.md)
- [2026-06-22](logs/2026-06-22.md)
- [2026-06-21](logs/2026-06-21.md)
- [2026-06-20](logs/2026-06-20.md)
- [2026-06-19](logs/2026-06-19.md)
- [2026-06-18](logs/2026-06-18.md)
- [2026-06-17](logs/2026-06-17.md)
- [2026-06-16](logs/2026-06-16.md)
- [2026-06-15](logs/2026-06-15.md)
